...
describe("Alarms", () => {
  ...
  it("tick before alarm time does not fire", async () => {
    ...
    await Effect.runPromise(runtime.task("job").send("j1", { _tag: "Start", delayMs: 10000 }))
  ...
  it("fireAlarm directly bypasses scheduling", async () => {
    ...
    await Effect.runPromise(runtime.task("job").send("j1", { _tag: "Start", delayMs: 999999 }))
    await Effect.runPromise(runtime.task("job").fireAlarm("j1"))
  ...
  it("multiple instances have independent alarms", async () => {
    ...
    await Effect.runPromise(runtime.task("job").send("fast", { _tag: "Start", delayMs: 1000 }))
    await Effect.runPromise(runtime.task("job").send("slow", { _tag: "Start", delayMs: 10000 }))
    ...
    expect(await Effect.runPromise(runtime.task("job").getState("fast"))).toEqual({
      status: "completed",
      attempts: 1,
    })
    expect(await Effect.runPromise(runtime.task("job").getState("slow"))).toEqual({
      status: "scheduled",
      attempts: 0,
    })
  })
