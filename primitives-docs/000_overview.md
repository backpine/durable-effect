...
This acts as a debounce to eat many unneeded transations in a system.
...
const { Jobs, JobsClient } = createDurableJobs({
  ...
  // WorkerPools
   webhookWorkerPool: WorkerPool.make({
...
### Simplification audit (current jobs package)

1. **Trim unimplemented debounce/workerPool surface area** — the runtime only routes continuous requests but the public API still exposes debounce/workerPool clients, request/response types, registry slots, and storage keys (`packages/jobs/src/runtime/dispatcher.ts:68-121`, `packages/jobs/src/client/client.ts:186-507`, `packages/jobs/src/runtime/types.ts`, `packages/jobs/src/storage-keys.ts`). This overpromises working features and bloats the bundle. Either hide these until handlers exist or gate them behind an explicit “experimental” flag.
...
6. **[DONE] Tighten status/metadata to active jobs** — pared the status union down to the continuous lifecycle values in use.
